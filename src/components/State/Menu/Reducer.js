...
const initialState = {
    menuItems: [], loading: false, error: null, search: [], message: null
}
...
const menuItemReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case actionTypes.CREATE_MENU_ITEM_SUCCESS:
            return { ...state, loading: false, menuItems: [...state.menuItems, action.payload], message: "Food created successfully" }
        case actionTypes.GET_MENU_ITEMS_BY_RESTAURANT_ID_SUCCESS:
            return { ...state, loading: false, menuItems: action.payload }
        case actionTypes.DELETE_MENU_ITEM_SUCCESS:
            return { ...state, loading: false, menuItems: state.menuItems.filter(menuItem => menuItem.id !== action.payload.id) };

        case actionTypes.UPDATE_MENU_ITEMS_AVAILABILITY_SUCCESS:
            console.log("updated items id: ", action.payload.id);
            return { ...state, loading: false, menuItems: state.menuItems.map(menuItem => menuItem.id === action.payload.id ? action.payload : menuItem) };
